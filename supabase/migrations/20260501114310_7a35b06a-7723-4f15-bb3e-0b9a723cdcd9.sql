-- Roles enum + table (kept separate from profiles to avoid privilege escalation)
...
CREATE POLICY "Admin write schedules" ON public.schedules FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
-- Moment photos (carousel, max 5 enforced in app)
...
CREATE POLICY "Admin write moment_photos" ON public.moment_photos FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
INSERT INTO public.app_settings (id, google_form_url, whatsapp_number)
VALUES (1, '', '085161302226')
